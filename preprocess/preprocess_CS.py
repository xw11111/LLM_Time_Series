...
def load_cs_metadata():
    ...
    try:
        diag_key = 'Chapman_Shaoxing/Diagnostics.csv'
        obj = s3_client.get_object(Bucket=bucket_name, Key=diag_key)

        df = pd.read_csv(io.BytesIO(obj['Body'].read()), encoding='utf-8-sig')
        print("Successfully loaded Diagnostics.csv from S3.")
    except Exception as e:
        print(f"S3 load failed ({e}); falling back to local Diagnostics.csv …")
        df = pd.read_csv('Diagnostics.csv', encoding='utf-8-sig')

    df.columns = df.columns.str.strip()

    required = {'FileName', 'Rhythm'}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Diagnostics.csv missing required columns: {missing}")

    df['Rhythm'] = df['Rhythm'].astype(str).str.strip().str.upper()
    ...
    df = df.dropna(subset=['Rhythm'])
    df['label_numeric'] = df['Rhythm'].map(numeric_map)

    df['FileName'] = df['FileName'].astype(str).str.strip()
    df['FileNameNoExt'] = df['FileName'].str.replace(r'\.csv$', '', regex=True)

    print("Label distribution of labels:")
...
df_main = load_cs_metadata()
print("ECG records in total in this dataset: ", len(df_main))

def read_ecg_csv_from_s3(file_stem: str, lead_idx: int):
    candidates = []
    fs = file_stem.strip()
    has_ext = fs.lower().endswith('.csv')
    stems = [fs, fs.replace('.csv','')] if has_ext else [fs]

    for s in stems:
        candidates.extend([
            f'Chapman_Shaoxing/ECGDataDenoised/{s}.csv',
            f'Chapman_Shaoxing/ECGData/{s}.csv',

            f'Chapman_Shaoxing/ECGDataDenoised/{s}',
            f'Chapman_Shaoxing/ECGData/{s}',
        ])

    for key in candidates:
        try:
            obj = s3_client.get_object(Bucket=bucket_name, Key=key)
            csv_text = obj['Body'].read().decode('utf-8', errors='ignore')
            df = pd.read_csv(io.StringIO(csv_text), header=None)
            num_leads = df.shape[1]
            if lead_idx < num_leads:
                return df.iloc[:, lead_idx].to_numpy()
            else:
                print(f"{key}: has {num_leads} leads < requested {lead_idx+1}")
                return None
    ...
    print(f"Could not find ECG CSV for '{file_stem}' in ECGData(Denoised)/.")
    return None
...
for file_index, (_, row) in enumerate(df_main.iterrows(), start=1):
    file_stem = str(row['FileNameNoExt'])
    data_label = int(row['label_numeric'])

    ecg_signal = read_ecg_csv_from_s3(file_stem, lead_idx=CONFIG["lead_index"])
    if ecg_signal is None:
        continue

    patient_id = f"CS_{file_stem}"
    try:
        p_wave, qrs_wave, t_wave, p_wave_indices, qrs_indices, t_wave_indices = process_ecg_signal(
        ...
        if not (p_wave and qrs_wave and t_wave and p_wave_indices and qrs_indices and t_wave_indices):
            ...
            continue
        if any(len(w) == 0 for w in [p_wave, qrs_wave, t_wave]):
