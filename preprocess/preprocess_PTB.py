...
ptb_csv_key = 'data/ptbxl/ptbxl_database.csv'
scp_csv_key = 'data/ptbxl/scp_statements.csv' 
ptb_df = read_csv_from_s3(bucket_name, ptb_csv_key)
scp_df = read_csv_from_s3(bucket_name, scp_csv_key)

diagnostic_scp = scp_df[scp_df['diagnostic'] == 1.0].copy()

scp_to_class = dict(zip(diagnostic_scp['Unnamed: 0'], diagnostic_scp['diagnostic_class']))

processed_records = []
dropped_multiple_different = 0

for idx, row in ptb_df.iterrows():
    ...
    active_labels = []
    for scp_code, value in scp_codes.items():
        if value > 0 and scp_code in scp_to_class:
            diagnostic_class = scp_to_class[scp_code]
            if diagnostic_class not in active_labels:
                active_labels.append(diagnostic_class)

    if len(active_labels) == 0:
        continue
    elif len(active_labels) == 1:
        record = row.copy()
        record['label'] = active_labels[0]
        processed_records.append(record)
    else:

        dropped_multiple_different += 1
        continue

df_main = pd.DataFrame(processed_records)

print(f"Original records: {len(ptb_df)}")
...
df_main['label_numeric'] = df_main['label'].map(label_mapping)
print(f"\nLabel mapping applied:")
for cat_label, num_label in label_mapping.items():
    count = (df_main['label'] == cat_label).sum()
    print(f"  {cat_label} -> {num_label} ({count} samples)")

if df_main['label_numeric'].isna().any():
    unmapped = df_main[df_main['label_numeric'].isna()]['label'].unique()
    print(f"Warning: Unmapped labels found: {unmapped}")
print(f"Numeric label distribution:")
print(df_main['label_numeric'].value_counts().sort_index())

df_main['filename'] = df_main['filename_lr'].apply(lambda x: x.split('/')[-1] if pd.notna(x) else x)

df_main = df_main[[
    "ecg_id","patient_id","recording_date","filename_lr","filename_hr",
    "scp_codes","label","label_numeric","strat_fold"
]].reset_index(drop=True)

print(f"Total records with labels: {len(df_main)}")
print(f"ECG ID range: {df_main['ecg_id'].min()} to {df_main['ecg_id'].max()}")
print("Sample filename_hr paths:")
for i, (_, row) in enumerate(df_main.head().iterrows()):
    print(f"  ECG_ID {row['ecg_id']}: {row['filename_hr']} -> Label: {row['label']}")

def read_ecg_data_from_dat(filename_hr):
    data_key = f'{base_prefix}{filename_hr}.dat'
    try:
        response = s3_client.get_object(Bucket=bucket_name, Key=data_key)
        dat_file = response['Body'].read()

        ecg_data = np.frombuffer(dat_file, dtype=np.int16).astype(np.float64)

        if len(ecg_data) % 12 == 0:
            ecg_data = ecg_data.reshape(-1, 12).T
        else:
            print(f"Warning: Data length {len(ecg_data)} not divisible by 12 leads")
        return ecg_data
    except Exception as e:
        print(f"Error reading {data_key}: {e}")
        return None
...
print("ECG records in total in this dataset: ", len(df_main))
...
for file_index, (_, row) in enumerate(df_main.iterrows(), start=1):
    ecg_id = row['ecg_id']
    data_label = row['label_numeric']
    filename_hr = row['filename_hr']

    ecg_data = read_ecg_data_from_dat(filename_hr)
    if ecg_data is None:
        continue

    if ecg_data.shape[0] > which_lead:
        ecg_signal = ecg_data[which_lead, :]
    else:
        print(f"Lead {which_lead} not available for ECG_ID {ecg_id}, using lead 0")
        ecg_signal = ecg_data[0, :]

    patient_id = f"ECG_{ecg_id:05d}"
    count = count + 1
    try: 
