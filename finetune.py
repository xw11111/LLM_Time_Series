...
parser.add_argument("--subsample_frac", type=float, default=1.0, help="Fraction of patients to keep in each split (e.g., 0.01 for 1%)")
...
max_seq_len = pretrain_params["max_seq_len"]
...
class BERTForClassification(nn.Module):
    ...
    def forward(self, input_ids, cnn_features, labels=None):
        pooled_output = self.bert(input_ids, cnn_features)
...
class ClassificationDataset(Dataset):
    ...
    def _process_sentence_cnn(self, cnn_sentence) -> torch.Tensor:
        zero_feat = torch.zeros(cnn_embed_dim, dtype=torch.float32)
        ...
        return torch.stack(cur_cnn, dim=0)

    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        lab_raw = row["label"]

        label_int = int(lab_raw) - (0 if CONFIG["dataset_name"]=="cs" else 1)
...
with open(beat_sentence_pickle_name, "rb") as f:
    patient_sent_dict = pickle.load(f)

with open(beat_sentence_cnn_pickle_name, "rb") as f:
    patient_sent_cnn_dict = pickle.load(f)
from collections import defaultdict

def stratified_subsample_patients(pids_set, labels_map, frac: float, seed: int):
    rng = np.random.RandomState(seed)
    by_class = defaultdict(list)
    for pid in pids_set:
        by_class[labels_map[pid]].append(pid)
    ...
    for cls, lst in by_class.items():
        ...
        lst = lst.copy()
        rng.shuffle(lst)
        k = max(1, int(round(frac * len(lst))))
        k = min(k, len(lst))
        keep.extend(lst[:k])
    return set(keep)
...
PATIENT_LABEL = {
    pid: int(patient_sent_dict[pid][-1]) - (0 if CONFIG["dataset_name"]=="cs" else 1)
...
if cli_args.subsample_frac < 1.0:
    frac = float(cli_args.subsample_frac)
    train_patients = stratified_subsample_patients(train_patients, PATIENT_LABEL, frac, SEED)
    val_patients   = stratified_subsample_patients(val_patients,   PATIENT_LABEL, frac, SEED)
    test_patients  = stratified_subsample_patients(test_patients,  PATIENT_LABEL, frac, SEED)
    print(f"Subsampled to fraction {frac:.3f} -> "
...
train_loader = DataLoader(train_dataset,batch_size=CONFIG["batch_size"],shuffle=True,drop_last=False,worker_init_fn=seed_worker,generator=g)
